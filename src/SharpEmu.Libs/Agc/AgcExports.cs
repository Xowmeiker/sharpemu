...
public static class AgcExports
{
    ...
    private static readonly HashSet<ulong> _dumpedShaderStates = [];
    ...
    // Malformed packets are dropped as completed so they cannot wedge the
    // hardware queue permanently.
        ...
        out string error)
    {
        ...
        DumpShaderStateIfSelected(exportShaderAddress, exportEvaluation);
        DumpShaderStateIfSelected(pixelShaderAddress, pixelEvaluation);
    ...
    private static void DumpShaderStateIfSelected(
        ulong shaderAddress,
        Gen5ShaderEvaluation evaluation)
    {
        if (_shaderInstructionDumpSuffix is not { } suffix ||
            (shaderAddress & 0xFFFFFuL) != suffix)
        {
            return;
        }
        ...
        lock (_submitTraceGate)
        {
            if (!_dumpedShaderStates.Add(shaderAddress))
            {
                return;
            }
        }

        var scalars = string.Join(
            ',',
            evaluation.InitialScalarRegisters
                .Take(64)
                .Select((value, index) => $"s{index}=0x{value:X8}"));
        var buffers = string.Join(
            ',',
            evaluation.GlobalMemoryBindings.Select((binding, index) =>
                $"b{index}:s{binding.ScalarAddress}:0x{binding.BaseAddress:X16}:" +
                $"{binding.DataLength}:" +
                Convert.ToHexString(
                    binding.Data.AsSpan(0, Math.Min(binding.DataLength, 128)))));
        var vertexInputs = string.Join(
            ',',
            (evaluation.VertexInputs ?? []).Select((binding, index) =>
                $"v{index}:pc0x{binding.Pc:X}:loc{binding.Location}:" +
                $"0x{binding.BaseAddress:X16}:stride{binding.Stride}:" +
                $"offset{binding.OffsetBytes}:c{binding.ComponentCount}:" +
                $"fmt{binding.DataFormat}/num{binding.NumberFormat}:" +
                Convert.ToHexString(
                    binding.Data.AsSpan(0, Math.Min(binding.DataLength, 128)))));
        Console.Error.WriteLine(
            $"[COMPAT][SHADER-STATE-DUMP] address=0x{shaderAddress:X16} " +
            $"scalars=[{scalars}] buffers=[{buffers}] vertex=[{vertexInputs}]");
    }
    ...
    private static ulong ComputeShaderStructuralFingerprint(Gen5ShaderEvaluation evaluation)
    {
        ...
        foreach (var binding in evaluation.GlobalMemoryBindings)
        {
            ...
            // Writable guest-VA aliases bind at an aligned descriptor offset,
            // so the translator bakes the discarded low address bits into their
            // accesses. Reusing that SPIR-V for a different byte bias reads the
            // wrong words and can make otherwise valid draw output flicker.
            Mix(GetGuestStorageBufferByteBias(binding));
    ...
    private static ulong ComputeShaderStateFingerprint(Gen5ShaderEvaluation evaluation)
    {
        ...
        // Keep the baked-scalar cache correct for the same descriptor-alignment
        // contract used by the normal structural cache.
        foreach (var binding in evaluation.GlobalMemoryBindings)
        {
            hash = (hash ^ GetGuestStorageBufferByteBias(binding)) * prime;
        }
    ...
    private static ulong GetGuestStorageBufferByteBias(
        ...
        IReadOnlyDictionary<uint, uint> registers)
    {
        ...
        // describes the hardware SGPR window even when no USER_DATA register
        // was written for the current draw. GFX10 NGG export shaders use the
        // GS user-data bank while their program address is carried in the
        // ES/NGG registers.
