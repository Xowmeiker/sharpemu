	...
	private static bool TryHandlePosixFault(int signal, nint siginfo, nint ucontext)
	{
		...
		if (disposition != -1 && !_posixSignalWarmup)
		{
			...
			try
			{
				...
				if (_posixSignalBackend is { } reporterBackend)
				{
					var windowStart = faultRip >= 0x20 ? faultRip - 0x20 : 0;
					var window = new byte[0x40];
					if (reporterBackend.TryReadHostBytes(windowStart, window))
					{
						Console.Error.WriteLine(
							$"[LOADER][ERROR]   code [rip-0x20..]: {Convert.ToHexString(window)}");
					}
					else if (reporterBackend.TryReadHostBytes(faultRip, window))
					{
						Console.Error.WriteLine(
