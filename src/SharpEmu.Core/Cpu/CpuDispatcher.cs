...
public sealed class CpuDispatcher : ICpuDispatcher, IDisposable
{
    ...
    // amd64 variant II); libc.prx alone reaches beyond -0x1700 and titles
    // with larger PT_TLS segments (Void Terrarium needs 0x1B00) exceed the
    // historical 4KB Windows prefix, so map a full 64KB everywhere.
    private const ulong TlsPrefixSize = 0x0001_0000UL;
        ...
        ulong programExitHandlerAddress)
    {
        ...
        if (!context.TryWriteUInt32(entryParamsAddress + 0x00, (uint)arguments.Count) ||
